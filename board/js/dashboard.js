...
$(document).ready(function() {
   ...
   connection.onopen = function (sess, details) {
      ...
      session.prefix("event", prefix);
      session.prefix("daimler", prefix);
      ...
      // daimler events
      session.subscribe("daimler:hi", onHI);
      session.subscribe("daimler:vital", onVital);
      session.subscribe("daimler:driver", onDriver);
      session.subscribe("daimler:accident", onAccident);
      session.subscribe("daimler:mood", onMood);
      session.subscribe("daimler:event", onEvent);
...
function initialize() {
   ...
   $("#revenue_threshold").slider({
      slide: function(event, ui) {
         ...
         session.publish("daimler:revenue-threshold-changed", [revenue_threshold]);
      }
   });
   ...
   $("#unit_threshold").slider({
      slide: function(event, ui) {
         ...
         session.publish("daimler:unit-threshold-changed", [unit_threshold]);
      }
   });
...
function DashboardViewModel () {
   ...
   this.hundred_bar_sections = ko.observableArray([
      new hundred_bar_section(100, hundredColor01, "Ruhig"),
      new hundred_bar_section(200, hundredColor02, "Moderat"),
      new hundred_bar_section(100, hundredColor03, "Aggresiv"),
   ]);
...
function drawPieChart () {
   ...
   pieChart("pieChart1", [parseInt(vm.pieSection01()), parseInt(vm.pieSection02()), parseInt(vm.pieSection03()), parseInt(vm.pieSection04())], 125, 170, 125, [chartColor01, chartColor02, chartColor03, chartColor04], ["Aggresiv", "Gelassen", "Unsicher", "Schläfrig"], 265, 0);
   pieChart("pieChart2", [parseInt(vm.pieSection01()), parseInt(vm.pieSection02()), parseInt(vm.pieSection03()), parseInt(vm.pieSection04())], 125, 170, 125, [chartColor01, chartColor02, chartColor03, chartColor04], ["Aggresiv", "Gelassen", "Unsicher", "Schläfrig"], 265, 0);
}
...
function onAccident (args, kwargs) {
   // session.log(topicURI, kwargs);
   if (kwargs["idx"]) {
      switch (kwargs["idx"]) {
         case 1:
...
function onEvent(args, kwargs) {
   session.log("onEvent", kwargs, kwargs.revenue > revenue_threshold);

   if (kwargs["revenue"] > revenue_threshold || kwargs["units"] > unit_threshold) {

      var icon = "sale";

      var text = kwargs["product"];
      if (!text) {
         text = "Hohe Unfallgefahr! Pause!"
      }

      vm.activity_stream_events.push(new activity_stream_event(text, kwargs["units"], kwargs["region"], kwargs["revenue"]));
      session.publish("daimler:activity-display-threshold-exceeded", [], kwargs);
